...
import { type Category } from "@/src/data/products"; // only the type, not the array
...
function ShoppingCharacter() {
  ...
  const armAngle = useTransform(scrollY,
    [0, 200, 400, 600, 800, 1000, 1200, 1400, 1600, 1800, 2000],
    [0, 20, -20, 20, -20, 20, -20, 20, -20, 20, 0]);
  const armSpring = useSpring(armAngle, { stiffness: 80, damping: 12 });

  const wheelRotate = useTransform(scrollY, [0, 2000], [0, 720]);
  const wheelSpring = useSpring(wheelRotate, { stiffness: 60, damping: 14 });

  const headTilt = useTransform(scrollY,
    [0, 300, 600, 900, 1200, 1500, 1800],
    [0, -8, 8, -5, 10, -8, 0]);
  const headSpring = useSpring(headTilt, { stiffness: 100, damping: 15 });

  const legFront = useTransform(scrollY,
    [0, 150, 300, 450, 600, 750, 900, 1050, 1200, 1350, 1500, 1650, 1800, 2000],
    [0, 28, -28, 28, -28, 28, -28, 28, -28, 28, -28, 28, -28, 0]);
  const legSpring = useSpring(legFront, { stiffness: 90, damping: 12 });
        ...
        >
          <svg width="96" height="120" viewBox="0 0 96 120" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <g transform="translate(52, 72)">
              ...
              <line x1="0" y1="4" x2="0" y2="0" stroke="#e5793a" strokeWidth="1.5" strokeLinecap="round" />
              <circle cx="10" cy="3" r="4" fill="#4ade80" />
              <circle cx="18" cy="2" r="3.5" fill="#f97316" />
              <circle cx="25" cy="4" r="3" fill="#fbbf24" />
              <motion.g style={{ rotate: wheelSpring, originX: "6px", originY: "6px" }}>
                <circle cx="6" cy="24" r="5" fill="white" stroke="#e5793a" strokeWidth="1.5" />
                <line x1="6" y1="20" x2="6" y2="28" stroke="#e5793a" strokeWidth="1" />
                <line x1="2" y1="24" x2="10" y2="24" stroke="#e5793a" strokeWidth="1" />
              </motion.g>
              <motion.g style={{ rotate: wheelSpring, originX: "26px", originY: "6px" }}>
                <circle cx="26" cy="24" r="5" fill="white" stroke="#e5793a" strokeWidth="1.5" />
                <line x1="26" y1="20" x2="26" y2="28" stroke="#e5793a" strokeWidth="1" />
                <line x1="22" y1="24" x2="30" y2="24" stroke="#e5793a" strokeWidth="1" />
              </motion.g>
            </g>
            ...
            <motion.g style={{ rotate: legSpring, originX: "34px", originY: "70px" }}>
              <rect x="28" y="70" width="9" height="26" rx="4.5" fill="#2d5a3d" />
              <ellipse cx="32" cy="96" rx="8" ry="4" fill="#1a3d28" />
            </motion.g>
            <motion.g style={{ rotate: useTransform(legSpring, v => -v), originX: "38px", originY: "70px" }}>
            ...
            <motion.g style={{ rotate: headSpring, originX: "34px", originY: "26px" }}>
              ...
              <circle cx="27" cy="22" r="2.8" fill="white" />
              <circle cx="41" cy="22" r="2.8" fill="white" />
              <circle cx="28" cy="23" r="1.5" fill="#3b1f0a" />
              <circle cx="42" cy="23" r="1.5" fill="#3b1f0a" />
              <circle cx="28.8" cy="22.2" r="0.6" fill="white" />
              <circle cx="42.8" cy="22.2" r="0.6" fill="white" />
              <path d="M27 29 Q34 35 41 29" stroke="#c0784a" strokeWidth="1.5" strokeLinecap="round" fill="none" />
              <circle cx="22" cy="28" r="4" fill="#f97316" opacity="0.25" />
              <circle cx="46" cy="28" r="4" fill="#f97316" opacity="0.25" />
        ...
        {heroImages.map((_, i) => (
          <button key={i} onClick={() => goTo(i)}
            className={`h-1.5 rounded-full transition-all ${
              active === i ? "w-5 bg-brand-orange" : "w-1.5 bg-stone-300"
            }`}
...
const { data, error } = await supabase
  .from("products")
      ...
      if (!error && data) setDbProducts(data as ProductRow[]);
      setLoading(false);
    }
    fetchProducts();
  }, []);
  ...
  const filtered = useMemo(
    ...
    [dbProducts, selectedCategory, search]
  );
  ...
  return (
    <div className="relative space-y-0 pb-36">
      ...
      <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl bg-gradient-to-br from-amber-50 via-orange-50 to-green-50 border border-orange-100 mb-10">
        ...
        <svg className="pointer-events-none absolute left-3 top-3 opacity-20"
          ...
          <div className="relative z-10 flex flex-col justify-center px-6 sm:px-10 py-10 sm:py-14">
            ...
            >
              ...
              <h1 className="font-display text-3xl sm:text-4xl md:text-5xl font-bold text-brand-charcoal leading-[1.08]">
                Fresh Groceries,<br />
                <em className="not-italic text-brand-orange">Curated for You.</em>
              </h1>
              ...
              <p className="max-w-sm text-sm sm:text-base text-stone-500 leading-relaxed">
                Hand-picked products from local farms and artisan producers.
                Browse, filter, and add to cart — our friend below will help carry them!&nbsp;🛒
              </p>
      ...
      {!loading && filtered.length === 0 && (
        ...
        >
          ...
          <p className="text-sm text-stone-400 max-w-xs">
            Try a different category or clear your search — there&apos;s plenty more!
          </p>
        </motion.div>
      )}
    </div>
